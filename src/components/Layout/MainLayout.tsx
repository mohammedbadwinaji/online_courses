...
export default function MainLayout() {
	...
	window.onmousedown = (ev) => {
		if (
			...
			(ev.target as HTMLElement).classList.contains("course")
		) {
			return false;
		}

		if (
			(ev.target as HTMLElement).classList.contains("openCategory") === false
		) {
			document.querySelector("ul.category")?.classList.add("hidden");
			document.querySelector("ul.subCategory")?.classList.add("hidden");
		} else {
			document.querySelector("ul.category")?.classList.remove("hidden");
		}
	};
	useEffect(() => {
		if (token) {
													...
													if (item.name === "Courses") {
														...
														return (
															<nav
																...
																className={
																	course
																		? "dark:bg-gray-900 dark:text-white bg-gray-400 text-white relative openCategory rounded-md px-3 py-2 text-sm font-medium transition  openCategory cursor-pointer"
																		: "dark:text-gray-300 text-gray-800  dark:hover:bg-gray-700 hover:bg-gray-300 dark:hover:text-white hover:text-white rounded-md px-3 py-2 text-sm font-medium transition relative openCategory cursor-pointer"
																}
																onMouseDown={() =>
																	document
															...
															>
																{item.name}
																<ul className="hidden absolute top-[50px] left-0 bg-slate-800 category">
																		...
																		return (
																			...
																			>
																				<NavLink
																					...
																					className={`course py-4 px-4 flex justify-center items-center w-[150px] ${
																						index !==
																						(item.category?.length as number) -
																							1
																								...
																								return (
																									...
																									>
																										<NavLink
																											...
																											className={`course py-4 px-4 flex justify-center items-center w-[200px] ${
																												index ===
